...
function CreateCompanyForm() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      cmpCd: Yup.string().required(
        "Company Code is required or must atleast 2 character long"
      ).min(2, 'Company Code must be at least 2 characters long'),
      cmpName: Yup.string().required("Company Name is required"),
      cmpAdd: Yup.string().required("Company Address is required"),
      ...
      faxNo: Yup.string()
        // .required("Fax no. should be 10 digits and cannot contain alphabets")
        .matches(
          /^\d{10}$/,
          "Fax no. should be exactly 10 digits and should not include any letters"
        ),
      offName: Yup.string().required("Office Name is required"),
      // offType: Yup.string().required("Office Type is required"),
      offAddress: Yup.string().required("Office Address is required"),
      // ctlCd: Yup.string().required("Control Code is required"),
    ...
    <Box>
      <form onSubmit={formik.handleSubmit}>
        <Card >
          <CardHeader
            title="New Company"
            subheader="OFFICE MANAGEMENT / NEW COMPANY"
          />
          <CardContent>
